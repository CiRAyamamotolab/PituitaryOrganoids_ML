...
for layer in conv_base.layers[:15]:#one-third of convolutional layers were  trained with our dataset.
    print(layer)
...
def grad_cam(input_model, x, layer_name):
    ...
    #scaling the image to the original image size
...
#Grad-CAM################################################################################
standardized_images = glob.glob(os.path.join(output_dir1,+"/*/*.jpg"))
for image_path in standardized_images:
  ...
  # Image Original 
  pic = x.reshape((1,) + x.shape)
  img_original=array_to_img(x)
  ...
  img_grad_cam=array_to_img(cam)
  plt.imshow(img_grad_cam)
  #plt.title(("fail, suc=",pred[idx]))
  plt.title("Grad-CAM")
  plt.axis('off')